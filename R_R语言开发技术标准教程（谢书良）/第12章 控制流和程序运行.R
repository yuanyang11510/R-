...
# R语言是一种计算机语言，可以通过编写自己需要的函数进行编程。将操作过程的命令代码写成程序，既便于保存，也便于以后使用。
    ...
    # 12.2 分支函数
        # 1. if-else函数
            ...
            sl12_4 <- function(year)
            {
                ...
                # 注意系统会提醒逻辑连接词分别使用重复的"&&"和"||"表示“且”和“或”
                {
                    print("是闰年。")
                }
                else
    ...
    # 12.4 流程转移
            ...
            sl12_15 <- function(n)
            {
                ...
                # 以上else()函数的一部分可以省略，因为如果if()函数的条件不满足，则会默认进入下一步操作（next）
                print(sumx)
            }
            sl12_15(1:100)
    ...
    # 12.5 程序运行
            ...
            # 实际上，可以进一步限制x、y、z的范围：如果100元全部用来买鸡翁，则可以买20只，相应的鸡母可以买33只（余一块），鸡雏可以买300只，
            # 因此x可以限制在0至20，y可以限制在0至33，z可以限制在0至100（因为鸡翁、鸡母和鸡雏的总数只有100）
            Num <- data.frame()
            for(x in 0:20)
            {
                for(y in 0:33)
