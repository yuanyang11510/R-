...
# 在R语言中，因子是用于对数据进行分类并将其存储为级别的数据对象。它们在具有有限数量的唯一值的列中很有用，如“男性”“女性”等。
    ...
    # 5.3 因子的转换及常见错误解决
            ...
            as.numeric(temperature) # 将因子转换为数值向量，但是此时的数值实际上是相应元素在原来因子中对应的因子水平（此处也可以看成是数值从小到大的排名）
    ...
    # 5.4 有序因子
            ...
            str1.order
            str2.order <- factor(str1, levels = c("D", "C", "B", "A"), ordered = TRUE) # 如果要自定义因子水平顺序为D<C<B<A，则需要使用factor()函数，并且表明参数ordered = TRUE
            str2.no.order <- factor(str1, levels = c("D", "C", "B", "A")) # 如果没有标明ordered = TRUE，则因子水平仅按照自定义的顺序排列，而不会通过小于号连接
            str2.order
            str2.no.order
            which(str2.order>="B") #which()函数返回满足相应条件的元素在向量中的索引值，此处即有序因子str2.order中大于等于B（即A和B）的元素在该因子中的索引值
            which(str1.order>"B") #如果选择有序因子str1.order，排序方式和str2.order相反，则str1.order中大于B的元素就变成了C和D，因此输出的是所有的值为C和D的元素在有序因子str1.order中的索引值
