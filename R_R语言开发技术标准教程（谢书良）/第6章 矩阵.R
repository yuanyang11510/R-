...
# R语言中,除向量之外,矩阵也是数据输入和计算的最简单形式。
    ...
    # 6.1 建立矩阵的函数matrix()
            a.matrix <- matrix(1:12,4) #除了明确行数为4，也可以明确列数为3，但是结合数据总数（12）和行数（4），可以推出列数为3，因此可以省略
            a.matrix
    ...
    # 6.3 将向量构成矩阵的函数
            ...
            x2
            rbind(x1,x2)
            # 注意按照行组合成矩阵要求每个元素的列数相同,否则虽然会输出结果,但是列数会取各元素中的最大值,列数小于该值的元素会从头重复各列的值
            ...
            x2
            cbind(x1,x2) 
            # 注意按照列组合成矩阵要求每个元素的行数相同,否则虽然会输出结果,但是行数会取各元素中的最大值,行数小于该值的元素会从头重复各行的值
