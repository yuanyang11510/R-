...
# 数组的含义是广义的。实际上向量就是一维数组，矩阵是二维数组，在这里所说的数组是指多维数组。当然，在这里介绍的内容也适用于向量和矩阵。
    ...
    # 7.3 apply函数
            ...
            # 注意，“维度名称”实际有两种指称，一种指的是某一维度上相应的维数所对应的名称（对应list每一行的对象内部的元素名称），另一种指的是整个维度的名称（对应list每一行的行名称），此处指的是后者，参考：https://stackoverflow.com/questions/6958041/r-apply-error-x-must-have-named-dimnames
