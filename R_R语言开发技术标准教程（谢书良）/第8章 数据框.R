...
# 向量（vector）、矩阵（matrix）或三维数组（array）其元素都是相同类型的数据，但在实际中常常需要处理不同类型的资料，例如学号、学生姓名、性别、年龄、身高、体重、各科成绩等，学号和年龄、身高、体重是数值，姓名和性别是字符串，各科成绩是向量，这些数据是无法放入同一个矩阵当中的。R语言提供了数据框（data frame）这样一种新的数据结构，可以很方便地解决这类问题。
    ...
    # 8.2 认识数据框的结构
            str(df.info)
            #* 书中说str()函数给出的结果中会将"Name"和"Sex"两列元素识别为因子，但是经过检验，无论是在RStudio还是在VS Code中，都已经简单地将这两列元素识别为字符串向量，原因是data.frame()函数已经将参数stringAsFactors默认设置为FALSE，可以通过以下命令进行检验：
            Name.df <- df.info[[1]]
            Sex.df <- df.info[[2]]
    ...
    # 8.3 数据框的引用
            df.info[1:2,3:5]
            #rfr 有关索引的内容可以参考文件"[(-)i]_[[i]]_[(-)i,]_[,(-)i].R"
