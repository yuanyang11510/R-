# 在names()、dimnames()函数中使用[i]、[[i]]
    ...
    # 一般说来,[i]/[[i]]只会出现在names()函数括号的外部,但是在列表或者数据框的情况下,可能会遇到需要使用[i]/[[i]]索引出列表或者数据框中第i行/列的元素/对象的需要,此时[i]/[[i]]就会同时出现在names()函数括号的内部,下面以列表为例,说明[i]/[[i]]出现在names()函数内外的各种情况
        ...
        # （二）[i]/[[i]]在names()函数外部：两种情况的结果相同
                ...
                #* 以上两者的结果相同，说明使用names()函数时并不会区分括号外的[i]和[[i]]，其效果均是默认将相应对象的第i个元素或者第i行/列的元素/对象的名称替换为相应的值,而将其余剩下的元素/数据/对象的名称替换为缺失值NA

    # 二、dimnames()函数
    #rfr 参考：https://www.dataanalytics.org.uk/row-column-names-using-dimnames-in-r/
    # dimnames()函数的作用是修改对象中的维度名称
    #rfr 注意，“维度名称”实际有两种指称，一种指的是某一维度上相应的维数所对应的名称（对应list每一行的对象内部的元素名称），另一种指的是整个维度的名称（对应list每一行的行名称），以下均是使用前者的指称，参考：https://stackoverflow.com/questions/6958041/r-apply-error-x-must-have-named-dimnames，关于维度名称和list的关系，详见下文
    ...
    # 和names()函数不同,在dimnames()函数的括号外使用[[i]]并不是用于索引相应对象中的第i个/第i列元素,并且也不能使用[i],详见下文
        ...
        # （一）[i]/[[i]]在dimnames()函数内部：两种情况的结果不同
                # （1）[[i]]在dimnames()函数内部
                dataframe1 <- data.frame(a=1:5,b=6:10,c=11:15)
                ...
                #* 在数据框的情况下，也可以从另一个角度想象一下：如果上述命令生效，将会在维度名称上产生冲突，即原数据框的行名称和列名称分别是1,2,3,4,5和a,b,c，而赋予新数据框的维度名称是a,b,c,d,e和C，显然两者是无法同时成立的，造成这种数据框独有的冲突的原因有两点：（1）数据框可以通过[i]索引出新的数据框；（2）dimnames()函数对数据框的行和列名称均会进行修改。原数据框和新数据框在列名称上是包含与被包含的关系，不会产生冲突，而在行名称上则是完全重合的，会产生冲突
                #* 如果要通过dimnames()函数修改原数据框第三列的名称，需要将[[2]]放到dimnames()函数的外面，而将[2]放到dimnames()函数的外面会报错，详见下文

        # （二）[i]/[[i]]在dimnames()函数外部：两种情况的结果不同
                # （1）[[i]]在dimnames()函数外部
                dataframe1 <- data.frame(a=1:5,b=6:10,c=11:15)
                ...
                #* 如上文所述，在dimnames()函数的括号外面只能使用[[i]]而不能使用[i]
