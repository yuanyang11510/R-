...
# （二）在列表和数据框中，
# [i]和[[i]]得到的分别是list的第i行数据/对象和dataframe的第i列的数据，
# 其中[i]给出的结果的类型保持不变，即仍然是一个list或者一个dataframe，
# 并且分别会给出该数据/对象在原list/dataframe中的行/列名称（如果设置了名称），

# 而[[i]]给出的结果的类型会改变，并且不会给出对应的行/列名称，
# 例如在dataframe中，[[i]]得到的是第i列数据组成的一个向量，
# 而在list中，[[i]]得到的是存储在第i行的数据/对象

# 顺带一提，如果list/dataframe设置了行/列名称，
# 也可以使用list/dataframe+$+行/列名称的方式进行索引，
# 结果和[[i]]一样，会导致索引结果的类型发生改变
...
# [i]可以索引多列，得到的是一个包含多行/多列数据的列表/数据框，
# 其中索引值可以写成1:i，c(1:i)，c(1,2,……,i)，但不能写成1,2,……,i
# 而[[i]]只能索引一列，原因也很好理解，
# 因为如果允许索引多列，将得到不同行/列数据组成的多个数据/对象/向量，这显然是不被允许的
# （想象如果要赋值，多个值无法赋给同一个变量）

# （三）注意，对于可以设置维度名称的矩阵和多维数组来说，
...
# 而矩阵和多维数组填充数据的方式是一次性填充而非分行/分列填充，[i]当然无法给出类似的信息
